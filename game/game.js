/* TOKENTOWN — cidade de pixel que cresce (sem parar) com os tokens da IA.
   ...
   - CONSTRUÇÃO AUTOMÁTICA: a cada 150k tokens a cidade ganha, SOZINHA, uma estrutura
     especial (sorteio ponderado). REGRA DA MEL: "sem queimar token, NENHUMA construção
     nova" — no boot/retomada a cidade nasce INTEIRA (todas as especiais devidas de uma
     vez, instantâneo); DEPOIS uma nova só surge quando ENTRA token real (delta>0). Ociosa
     (delta 0) = cidade estática: dia/noite, clima e habitantes seguem, mas ZERO prédio novo.
...
(function () {
  ...
  // ---------- fonte de tokens ----------
  var SIM_BURN = 320;             // tokens/seg simulados (só no navegador)
  var TOK_PER_BUILD_SIM  = 2500;
  ...
  var SUF = ['','k','M','B','T'];
  // pt-BR: vírgula decimal como no site. >=1M -> sempre 1 casa (27,2M; 1,3M), some ",0" (2M).
  // milhares -> comportamento antigo (só 1 casa quando <10), com vírgula quando houver decimal (4,7k).
  function fmt(n){ if(!(n>=1000)) return Math.floor(n>0?n:0).toString();
    ...
    return s.replace('.', ',') + SUF[t]; }
  // população: pt-BR com vírgula decimal (ex.: 12,4k). Uma casa quando < 100 no sufixo.
  function fmtPop(n){ n = Math.max(0, Math.floor(n||0)); if(n<1000) return String(n);
    var t=0, x=n; while(x>=1000 && t<SUF.length-1){ x/=1000; t++; }
    var s = (x<100) ? x.toFixed(1) : String(Math.round(x));
    if(s.slice(-2)==='.0') s = s.slice(0,-2);
    return s.replace('.', ',') + SUF[t]; }
  function fmtCost(v){ return '≈ US$ ' + (v||0).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}); }
  ...
  function setState(st){
    ...
    if(elLiveTxt) elLiveTxt.textContent = st==='live' ? 'ao vivo' : (st==='decision' ? 'sua decisão' : 'ocioso');
    // botão do recreio só aparece quando o agente está AO VIVO (ou já jogando).
    if(elRecreio) elRecreio.hidden = !(st==='live' || mode==='recreio');
  }

  // ---------- AVISO ATIVO (som + pulso da borda) na TRANSIÇÃO de estado ----------
  // A Mel: "quando o agente para, o app demora e não me avisa quando preciso decidir
  // ou agir — fica parado me esperando". Além do LED (setState) e da Notification do
  // sistema (main.js), na transição p/ 'decision'/'idle' tocamos um chime GENTIL (Web
  // Audio SINTETIZADO — sem asset, sem dep) e pulsamos a borda ~3s. Uma vez por
  // transição (debounce no onUsage). O SOM não depende de prefers-reduced-motion (não é
  // movimento); o PULSO respeita (sem pulso, mantém notificação + LED).
  var _actx = null;
  function audioCtx(){
    if(_actx) return _actx;
    // property access (não identificador cru) -> undefined-safe fora do browser (testes headless).
    var AC = window.AudioContext || window.webkitAudioContext;
    if(!AC) return null;
    try { _actx = new AC(); } catch(e){ return null; }
    return _actx;
  }
  // chime BAIXO e curto: 2 notas subindo p/ 'decision' (mais chamativo), 1 nota p/ 'idle'.
  function chime(kind){
    var ctx2 = audioCtx();
    if(!ctx2) return;
    try {
      ...
      var notes = kind==='decision'
        ? [[659.25, 0, 0.06], [987.77, 0.13, 0.06]]  // Mi5 -> Si5 (sobe: "preciso de você")
        : [[523.25, 0, 0.05]];                         // Dó5 único (calmo: "terminei")
      for(var i=0;i<notes.length;i++){
        var f=notes[i][0], at=t0+notes[i][1], vol=notes[i][2];
        var osc=ctx2.createOscillator(), g=ctx2.createGain();
        osc.type='sine'; osc.frequency.value=f;
        g.gain.setValueAtTime(0.0001, at);                        // ramp exponencial não aceita 0
  ...
  // pulso da borda ~3s (âmbar p/ decision, teal neutro p/ idle). Respeita reduced-motion.
  function pulseBorder(kind){
    if(reduce || !elCard || !elCard.classList) return;
    var cls = kind==='decision' ? 'pulse-decision' : 'pulse-idle';
    elCard.classList.remove('pulse-decision','pulse-idle');
    void elCard.offsetWidth;                                       // reflow: reinicia a animação
    elCard.classList.add(cls);
    if(_pulseTimer) clearTimeout(_pulseTimer);
    _pulseTimer = setTimeout(function(){ if(elCard && elCard.classList) elCard.classList.remove(cls); }, 3200);
  }
  // aviso da transição: som + pulso juntos (a Notification do sistema fica no main.js).
  function alertTransition(kind){ chime(kind); pulseBorder(kind); }
  ...
  function rebuildCity(){
    ...
    syncSpecialsInstant();                   // cidade nasce com TODAS as especiais devidas (instantâneo, sem gotejo)
  ...
  // ---------- CONSTRUÇÃO AUTOMÁTICA: só ergue estrutura quando ENTRA token real ----------
  // REGRA DA MEL: "sem queimar token, NENHUMA construção nova". As especiais são uma FUNÇÃO
  // PURA dos tokens (1 a cada SPECIAL_STEP). Nada de gotejo por TEMPO (o antigo DRAIN_MS
  // erguia prédios mesmo ociosa — violava a regra e foi removido). Boot/rebuild = instantâneo;
  // depois, incremental só no déficit criado por token novo. Ociosa => owed não muda => zero.
  function specialsOwed(){ return Math.floor(tokens() / SPECIAL_STEP); }
  // catch-up INSTANTÂNEO (boot/rebuild): só REGISTRA as especiais devidas no store (o
  // rebuildCity recoloca as últimas na orla). Sem recompensa/nota — é o nascimento da cidade.
  function syncSpecialsInstant(){
    var owed = specialsOwed(), added = 0;
    while(store.specials.length < owed){ store.specials.push(pickSpecialType(store.specials.length)); added++; }
    if(added) saveStore();
    return added;
  }
  // construção INCREMENTAL (por frame): ergue na orla APENAS o déficit que tokens reais
  // acabaram de criar. Sem déficit (ociosa) => NADA. Cap por frame só pra um salto pequeno
  // não pipocar tudo no mesmo quadro (saltos grandes já viram rebuild instantâneo).
  function buildSpecials(now){
    var owed = specialsOwed();
    if(store.specials.length >= owed) return;      // ociosa / nada devendo -> ZERO construção
    var made = 0;
    while(store.specials.length < owed && made < 8){
  ...
  function initSeason(sid){
    ...
    lastNote=0; lastCitySig=''; lastCitySent=0;
  ...
  // ---------- entrada de dados ----------
  var usageSeen = false; // 1º payload estabelece baseline em silêncio (sem chime no boot)
  if (window.tt && window.tt.onUsage) {
    ...
    window.tt.onUsage(function (d) {
      ...
      var prevState = agState;                          // estado anterior (p/ detectar a transição)
      agState = d.state || (d.live ? 'live' : 'idle'); // compat: main antigo só manda o booleano
      liveNow = (agState === 'live');
      // AVISO ATIVO na TRANSIÇÃO (1x): som + pulso quando ela precisa decidir/agir. O
      // 1º payload só fixa o baseline (usageSeen) — evita chime a cada relançamento do
      // app; só transições DENTRO da sessão avisam. reduced-motion: só o pulso é omitido.
      if (usageSeen && agState !== prevState && (agState === 'decision' || agState === 'idle'))
        alertTransition(agState);
      usageSeen = true;
  ...
  // ---- DIRIGÍVEL raro: a cada 30-60min cruza o céu devagar; banner "TOKENTOWN" bem legível ----
  ...
  // LEGIBILIDADE (a Mel: "não dá pra ler nada nele"): dirigível MAIOR (58x16) com uma PLACA
  // escura de alto contraste atravessando o corpo e "TOKENTOWN" em fonte pixel maior (bold 9px)
  // com CONTORNO escuro — legível contra o céu de dia ou de noite. À noite o banner ainda brilha.
  // Segue RARO (updateBlimp intacto: 30-60min) e não toca em mais nada da cidade calma.
  function drawBlimp(night){
    if(!blimp) return; var x=blimp.x|0, y=blimp.y|0, BW=58, BH=16;
    var body = night? '#6a5f7a' : '#8a7f96';
    // ENVELOPE (corpo) com cantos suavizados, brilho no topo e sombra embaixo
    R(x+1, y, BW-2, BH, body);
    R(x, y+2, 1, BH-4, body); R(x+BW-1, y+2, 1, BH-4, body);       // laterais arredondadas
    R(x-2, y+BH/2-2, 2, 4, body); R(x+BW, y+BH/2-2, 2, 4, body);   // bico e cauda
    R(x+2, y+1, BW-4, 1, mix(body,'#fff',0.18));                   // brilho superior
    R(x+2, y+BH-2, BW-4, 1, mix(body,'#000',0.28));               // sombra inferior
    R(x+BW-1, y+1, 3, 3, body); R(x+BW-1, y+BH-4, 3, 3, body);     // lemes da cauda
    R(x+(BW>>1)-4, y+BH, 8, 3, mix(body,'#000',0.35));            // gôndola
    // PLACA (banner) escura + "TOKENTOWN" com contorno — alto contraste, sem estourar a placa
    var bx=x+3, bw2=BW-6, byT=y+3, bh=10;
    R(bx, byT, bw2, bh, night? '#211a30' : '#28223a');            // fundo escuro da placa
    R(bx, byT, bw2, 1, mix(body,'#fff',0.12));                    // filete claro no topo
    R(bx, byT+bh-1, bw2, 1, '#120d1f');                          // filete escuro embaixo
    var lit = night ? '#ffe6a8' : '#ffdca6', ink = night ? '#120d1f' : '#171320';
  ...
  // ==========================================================================
  // SETUP (Fase 3 / A4) — chega por IPC onSetup; o MAIN é quem coleta (collectSetup)
  // e o manda pro SITE (o site é quem mostra quais skills/MCP foram usados).
  // A CIDADE do app NÃO desenha mais NADA de setup: a Mel pediu a cidade LIMPA —
  // "sem nome no app". As placas de skill nomeadas (COPY-MEL/…), as faixas de
  // destaque, os faroletes e as estações MCP com letreiro POLUÍAM a cena e
  // apareciam SEMPRE; saíram. A cidade volta a ser exatamente a movida por tokens
  // (como era antes do A4). applySetup fica INÓCUO: só guarda o último payload;
  // nenhum desenho é feito a partir dele (o dado vive só no site, via o main).
  // ==========================================================================
  var setupData = null;
  // intake do IPC (onSetup). null LIMPA; objeto guarda; ausente/inválido preserva.
  // NÃO desenha (cidade limpa) — collectSetup/reporter no main seguem intocados.
  function applySetup(d){
    if(d === null){ setupData = null; return; }
    if(!d || typeof d !== 'object') return;
    setupData = d;
  }
  if (window.tt && window.tt.onSetup) { window.tt.onSetup(function(d){ applySetup(d); }); }
  ...
  function draw(dt){
    ...
    drawBackline(n);
    // prédios da frente (normais + especiais) — cidade LIMPA (sem placas de skill)
    for(var f=0;f<city.length;f++){ var b=city[f]; if(b.rise<1) b.rise=Math.min(1,b.rise+dt/480);
      ...
      if(b.kind==='special'){ drawSpecial(b,sx,n); }
      else { drawNormal(b,sx,n); } }
  ...
  // ============================================================================
  var mode = 'city', rc = null, rcBest = 0;
  try { rcBest = parseInt(localStorage.getItem('tt-recreio-best'), 10) || 0; } catch(e){}
  // ---------- SEED DA PARTIDA (runSeed): mapa NOVO a cada entrada no recreio ----------
  // Cada ENTRADA no recreio sorteia um mundo FRESCO (Math.random funciona no renderer).
  // DENTRO da partida o runSeed é FIXO: rcRespawn (voltar pra bandeira) e o restart pós-
  // queda regeneram o MESMO trecho. Só um NOVO run (▶ recreio de novo / reset pós-ESC)
  // troca o runSeed. A identidade visual (paleta noturna, corpo/telhado, altura média)
  // NÃO depende do runSeed — muda só o LAYOUT/sequência. A cidade calma segue determinística
  // pela seed da temporada (store.seed), intocada.
  var rcForcedSeed = null;                    // testes fixam o runSeed do PRÓXIMO run (window.__rc.setRunSeed)
  function rcNewRunSeed(){
    if(rcForcedSeed != null){ var s = rcForcedSeed >>> 0; rcForcedSeed = null; return s; }
    return (Math.random()*4294967296) >>> 0;  // 32-bit fresco por partida
  }
  ...
  // TEMA POR PARTIDA (derivado do runSeed): muda o CARÁTER VISUAL do recreio — tom dos
  // telhados/corpo/céu, ornamento do topo, calor das janelas e o arranjo dos tokens da
  // abertura. É PURAMENTE COSMÉTICO/ESTRUTURAL na apresentação: NÃO toca física, colisão
  // nem rcSpec (dificuldade). Faz cada run "parecer outra cidade" já no 1º telhado.
  function rcMakeTheme(runSeed){
    var r = seededRand((runSeed ^ 0x51ed270b) >>> 0);
    var T = [
      { body:'#2a6a7a', roof:'#3aa0b0', sky:'#123040', k:0.16, kr:0.24, ks:0.16, name:'teal'    },
      { body:'#7a3a6a', roof:'#c05a8a', sky:'#2a1030', k:0.16, kr:0.24, ks:0.16, name:'magenta' },
      { body:'#7a5a2a', roof:'#c08a3a', sky:'#2a1e10', k:0.14, kr:0.22, ks:0.14, name:'amber'   },
      { body:'#3a4a8a', roof:'#5a6ac0', sky:'#101830', k:0.16, kr:0.24, ks:0.16, name:'indigo'  },
      { body:'#3a7a4a', roof:'#5ab06a', sky:'#0e2418', k:0.14, kr:0.22, ks:0.14, name:'jade'    },
      { body:'#8a4a3a', roof:'#c06a4a', sky:'#2a1410', k:0.14, kr:0.22, ks:0.14, name:'rust'    } ];
    var idx = (runSeed >>> 0) % T.length, t = T[idx];
    return { idx: idx, name: t.name, body: t.body, roof: t.roof, sky: t.sky, k: t.k, kr: t.kr, ks: t.ks,
      ornament: (r() < 0.5) ? 'crown' : 'ledge',   // topo dos telhados: coroa nas pontas x faixa clara
      winShift: 0.10 + r()*0.16,                    // quanto a janela puxa pro tom do tema
      tokPat: Math.floor(r()*3) };                  // 0 linha / 1 arco / 2 zig — arranjo dos tokens da abertura
  }
  // TOPO CRU de um telhado gerado (o 2º sorteio do stream determinístico daquele bi, na MESMA
  // ordem de rcSpec). Serve pra clearability saber a altura do telhado de DESTINO (bi+1) sem
  // reconstruir a spec inteira — o vão de `bi` leva a `bi+1`, então quem manda no teto é bi+1.
  function rcTopRaw(runSeed, bi){
    var r = seededRand((runSeed ^ Math.imul(bi+9, 0x9e3779b9)) >>> 0);
    r();                                     // descarta o sorteio de w (rcSpec tira w antes de top)
    return 32 + Math.floor(r()*10);
  }
  // TETO DE VÃO por SUBIDA (clearability — "todo pulo alcançável com margem, não frame-perfect").
  // Quando o próximo telhado é MAIS ALTO, a parábola do pulo pousa mais cedo (menos tempo no ar ->
  // menos alcance horizontal): subida maior => vão menor, pra caber com FOLGA num pulo SEGURADO
  // moderado (~5 quadros de hold, ~85% da velocidade, timing não-frame-perfect). Os limites vêm da
  // SIMULAÇÃO da física real (ver test-game.js "clearability", que reprova qualquer transição que
  // só passe com input perfeito); aqui fica ~2 de mundo abaixo do limite medido (margem). Descidas
  // e vãos planos NÃO passam por aqui — seguem variados até 17 (o jogo continua desafiador).
  function rcRiseCap(rise){
    if(rise <= 2) return 17;                 // subida pequena: vão cheio
    if(rise <= 4) return 16;
    if(rise <= 6) return 15;
    if(rise <= 7) return 13;
    if(rise <= 8) return 12;
    return 11;                               // subida de 9 = desnível máx entre telhados (topos 32..41)
  }
  ...
  // curto-circuito) p/ a sequência ser estável e a rampa, previsível. Pura em (runSeed, bi):
  // o teto de vão por subida olha o telhado de DESTINO (bi+1) via rcTopRaw — não toca sorteios.
  function rcSpec(runSeed, bi){
    ...
    // CLEARABILITY: o vão deste telhado leva ao telhado bi+1. Se ele for MAIS ALTO (subida), a
    // parábola pousa mais cedo (menos alcance) -> encolhe o vão em função da subida pra caber com
    // margem num pulo segurado moderado (nunca exige input perfeito). Destino = rcTopRaw(bi+1).
    var rise = top - rcTopRaw(runSeed, bi + 1);                    // >0 => bi+1 é mais alto (subida)
    if(rise > 0){ var rc_cap = rcRiseCap(rise); if(gap > rc_cap) gap = rc_cap; }
    ...
    // CHECKPOINT (bandeirinha) a cada ~15 telhados — derivado de (runSeed,bi) SEM tocar no stream
    // r() acima (rampa de dificuldade intacta). Fase pela seed; nunca no começo (di<15).
    var flag = (di >= 15) && ((di % 15) === (runSeed % 15));
    // ZONA SEGURA DO CHECKPOINT: no telhado da bandeira (onde a jogadora RENASCE) e no
    // imediatamente seguinte (o 1º passo do respawn) NÃO pode haver perigo que mata — senão
    // ela reaparece em cima de um bug/drone e morre na hora. `afterFlag` = este telhado vem
    // logo depois de uma bandeira. É só OVERRIDE (não consome r()) -> determinismo e rampa de
    // dificuldade intactos; bandeiras são >=15 apart, então nunca colidem entre si.
    var afterFlag = ((di - 1) >= 15) && (((di - 1) % 15) === (runSeed % 15));
    if(flag || afterFlag){ bug = false; drone = null; }
    // ESCUDO DE CACHE: ~metade dos blocos ? soltam o computadorzinho no lugar do token. Hash
    // INDEPENDENTE (não consome r()) -> determinístico e sem alterar nenhuma outra spec.
    var shield = block && (seededRand((runSeed ^ Math.imul(bi+31, 0x85ebca6b)) >>> 0)() < 0.5);
  ...
  function rcGenSeg(){                       // constrói 1 telhado + itens + desafios a partir da spec
    var s = rcSpec(rc.runSeed, rc.bi), x = rc.frontierX, w = s.w, top = s.top;
    ...
    if(s.drone) rc.drones.push({ x0:x-2, x1:x+w+2, x:x+(w>>1), dir:s.drone.dir,
  ...
  function rcBuild(){
    ...
    rc.theme = rcMakeTheme(rc.runSeed >>> 0);               // TEMA da partida (visual; regenera igual na mesma run)
    // ABERTURA SORTEADA pelo runSeed (fim do berço hardcoded): largura do berço, sacada,
    // bloco ?, tokens e as alturas/larguras/vãos de P1/P2 mudam JÁ nos 1os telhados a cada
    // partida. Invariantes de JOGABILIDADE preservados: berço no topo 40 (spawn/reset/coyote
    // dos testes), sacada cobre a coluna do spawn (x=12) com topo <36 (one-way testável), há
    // um VÃO real logo após o berço (andar sem pular = cair) e o 1º pulo é confortável.
    var o = seededRand((rc.runSeed ^ 0x1234abcd) >>> 0), BT = 40;
    var bw = 46 + Math.floor(o()*22);                       // berço 46-67 de largura (silhueta varia)
    rc.plats.push({ x:0, w:bw, top:BT, thin:false });       // P0: telhado-berço
    var sx = 4 + Math.floor(o()*4); if(sx > 8) sx = 8;      // sacada começa 4-8
    var sw = Math.max(16 - sx, 12 + Math.floor(o()*6));     // largura garante cobrir x=12..16
    var stp = 28 + Math.floor(o()*3);                       // topo da sacada 28-30 (<36; passa por baixo, pousa em cima)
    rc.plats.push({ x:sx, w:sw, top:stp, thin:true });      // sacada suspensa (one-way) sobre o berço
    var qx = Math.max(20, Math.min(bw-16, 28 + Math.floor(o()*14)));  // bloco ? DENTRO do berço (alcançável de baixo)
    rc.blocks.push({ x:qx, w:12, y:BT-20, h:8, used:false });
    var tp = rc.theme.tokPat, tn = 2 + Math.floor(o()*2);   // tokens no ar sobre o berço (arranjo do TEMA)
    for(var tkI=0; tkI<tn; tkI++){ var txk = 14 + tkI*7 + Math.floor(o()*4);
      var tyk = (BT-8) + (tp===1 ? -(4 - Math.abs(tkI-1)*3) : tp===2 ? (tkI%2?4:-3) : -tkI);
      if(txk < bw-4) rc.toks.push({ x:txk, y:tyk, got:false }); }
    var g1 = 10 + Math.floor(o()*4);                        // VÃO após o berço 10-13
    var p1x = bw + g1, p1w = 28 + Math.floor(o()*12);
    var p1t = Math.max(32, Math.min(44, BT + (Math.floor(o()*13) - 6)));  // P1 topo 34-44 (desnível <=6 -> pulo confortável)
    rc.plats.push({ x:p1x, w:p1w, top:p1t, thin:false });   // P1 depois do vão
    rc.toks.push({ x:p1x + (p1w>>1), y:p1t-8, got:false });
    // COMPUTADOR CEDO E GARANTIDO: o 2º bloco ? da partida (em P1, telhado limpo logo após o
    // 1º vão) SEMPRE solta o escudo. Assim toda partida oferece o power-up cedo, num ponto
    // alcançável (bloco 20 acima do telhado -> item pousa a restY=p1t-24, dentro do pulo),
    // sem depender de sorte. O bloco do berço (P0) segue soltando token dourado (aprende a bater).
    var p1cx = p1x + (p1w >> 1);
    rc.blocks.push({ x:p1cx - 6, w:12, y:p1t - 20, h:8, used:false, shield:true });
    var g2 = 11 + Math.floor(o()*4);                        // outro vão 11-14
    var p2x = p1x + p1w + g2, p2w = 26 + Math.floor(o()*12);
    // P2 topo 32-41: teto em 41 (não 44) pra a EMENDA P2->1º telhado gerado (topo 32-41) nunca
    // virar uma subida injusta (>9). Assim a clearability vale já na entrada da cidade gerada.
    var p2t = Math.max(32, Math.min(41, p1t + (Math.floor(o()*13) - 6)));
    rc.plats.push({ x:p2x, w:p2w, top:p2t, thin:false });   // P2 depois de outro vão
    rc.frontierX = p2x + p2w;
    rcGen();
    rc.px = 12; rc.py = BT; rc.onGround = true;              // nasce sobre o berço (P0)
  }
  ...
  function startRecreio(){
    ...
    mode='recreio';
    rc = { t:0, runSeed: rcNewRunSeed() };        // mapa NOVO por partida (não vem da seed da cidade)
    rcBuild();
  ...
  try { if(typeof window!=='undefined') window.__rc = {
    spec: rcSpec, diff: rcDiff, theme: rcMakeTheme,          // tema por partida (visual, derivado do runSeed)
    state: function(){ return rc; },                         // estado vivo (arrays de entidades)
    land: function(px, pf, ft, l){ return rcLanding(px, pf, ft, l); },
    setRunSeed: function(n){ rcForcedSeed = (n>>>0); } };    // fixa o runSeed do PRÓXIMO run (reprodutibilidade nos testes)
  } catch(e){}
  ...
  function rcDie(msg){ if(rc.dead) return; rc.dead=true; rc.deadT=0; rc.deadMsg=msg||'você caiu';
  ...
  function rcBonk(px, prevHead, head){        // bater com a CABEÇA por baixo do bloco ? -> +token
    ...
    for(var b=0;b<rc.blocks.length;b++){ var q=rc.blocks[b], bot=q.y+q.h;
      if(hx1>q.x && hx0<q.x+q.w && prevHead>=bot && head<=bot){
        // ESCUDO JÁ ABERTO: o bloco fica VAZADO por baixo pra a jogadora pular ATRAVÉS e
        // alcançar o computador que pairou logo acima. Sem isso o próprio bloco (que fica ~20
        // de mundo acima do telhado, mais alto que o ápice do pulo) travaria a cabeça e o item
        // nunca dava pra pegar. Bloco de token comum segue SÓLIDO por baixo (clássico do ?).
        if(q.used && q.shield) continue;
        ...
        if(!q.used){ q.used=true;
          if(q.shield){                                      // ESCUDO DE CACHE: sai o computadorzinho (em vez de token)
            // paira logo acima do bloco (restY = q.y-2); com o bloco agora vazado, o pulo reto
            // dali chega ao ápice (~telhado-13) e a hitbox generosa (Δy<11) fecha a pegada.
            rc.items.push({ x:q.x+q.w/2, y:q.y, y0:q.y, restY:q.y-2, age:0 });
  ...
  function rcUpdate(dt){
    ...
    for(var oi=0;oi<rc.obst.length;oi++){ var o=rc.obst[oi];
    ...
    for(var it=rc.items.length-1; it>=0; it--){ var im=rc.items[it]; im.age += dt;
      ...
      // HITBOX GENEROSA (Δx<7, Δy<11 de mundo): o computador é o power-up amigável — encostar
      // perto no ápice do pulo já pega; ele fica pairando até sair de cena (não some rápido).
      if(Math.abs(im.x-rc.px)<7 && Math.abs(im.y-(rc.py-4))<11){ rc.shield=true;
    ...
    if(!rc.dead && rc.iframes<=0) for(var bi2=0;bi2<rc.bugs.length;bi2++){ var b2=rc.bugs[bi2]; if(b2.dead) continue;
      if(rc.px+PW/2 > b2.x-2 && rc.px-PW/2 < b2.x+2 && rc.py > b2.top-4 && rc.py-PH < b2.top){ rcHurt('o bug te pegou'); break; } }
    //  · drone (perigo aéreo) em QUALQUER toque — não é stompável.
    if(!rc.dead && rc.iframes<=0) for(var di2=0;di2<rc.drones.length;di2++){ var d3=rc.drones[di2];
      if(rc.px+PW/2 > d3.x-3 && rc.px-PW/2 < d3.x+3 && rc.py > d3.y-2.5 && rc.py-PH < d3.y+2.5){ rcHurt('o drone te pegou'); break; } }
    // CAIU num vão -> perde; escudo NÃO salva de cair (só de bug/drone). Volta suave.
    if(rc.py > RWATER) rcDie('você caiu');
  }
  ...
  function drawRecreio(dt){
    ...
    var cam = rc.camX;
    // TEMA da partida (cosmético): tinge céu/corpo/telhado/janela e escolhe o ornamento do topo.
    var TH = rc.theme || (rc.theme = rcMakeTheme((rc.runSeed||0) >>> 0));
    var winCol = mix('#ffcf7a', TH.roof, TH.winShift);       // calor das janelas puxado pro tom do tema
    var ornHi  = mix(TH.roof, '#ffffff', 0.35);              // brilho do ornamento (parte HEX -> mix simples)
    ...
    // fundo noturno + estrelas (skyline aceso) — céu levemente tingido pelo tema
    var bandH=Math.ceil((RBASE*RZ)/SKY_N.length);
    for(var i=0;i<SKY_N.length;i++) R(0,i*bandH,W,bandH+1, mix(SKY_N[i], TH.sky, TH.ks));
    ...
    for(var p=0;p<rc.plats.length;p++){ var pl=rc.plats[p];
      ...
      if(pl.thin){ WR(pl.x, pl.top, pl.w, 1.5, mix(BODY[3], TH.body, TH.k)); WR(pl.x, pl.top-0.5, pl.w, 0.5, mix(ROOF[2], TH.roof, TH.kr));
        for(var wxa=pl.x+2; wxa<pl.x+pl.w-2; wxa+=4) WR(wxa, pl.top+0.5, 0.5, 0.5, winCol); }
      else { WR(pl.x, pl.top, pl.w, RBASE-pl.top, mix(BODY[((pl.x/5|0)%BODY.length+BODY.length)%BODY.length], TH.body, TH.k));
        WR(pl.x, pl.top-1, pl.w, 1, mix(ROOF[((pl.x/7|0)%ROOF.length+ROOF.length)%ROOF.length], TH.roof, TH.kr));
        WR(pl.x, pl.top, 0.5, RBASE-pl.top, mix(BODY[3],'#fff',0.1));
        // ORNAMENTO do TEMA no topo (cosmético; acima da linha do telhado, NÃO colide)
        if(TH.ornament==='crown'){ WR(pl.x, pl.top-2, 1.5, 1.5, ornHi); WR(pl.x+pl.w-1.5, pl.top-2, 1.5, 1.5, ornHi); }
        else { WR(pl.x, pl.top-1.5, pl.w, 0.5, ornHi); }
        for(var wy=pl.top+3; wy<RBASE-2; wy+=4.5) for(var wx=pl.x+2; wx<pl.x+pl.w-2; wx+=4)
          WR(wx,wy,1.5,1.5,winCol); } }
    // água
    ...
    ctx.fillStyle='#ffd79a'; ctx.textAlign='left';  ctx.fillText('recreio · '+rc.score+' tokens', 4, 2);
    ctx.fillStyle='#7fc7bf'; ctx.textAlign='right'; ctx.fillText('recorde '+rcBest, W-4, 2);
    ...
    if(rc.introT < 4000 && !paused && !rc.dead) rcStrip('← → andam · espaço pula · esc sai');
    // PAUSA — dois recados DIFERENTES: decisão pendente (âmbar) x agente terminou
    if(paused){
      if(agState==='decision') rcPanel('⏸ precisa da sua decisão', 'o agente está te esperando', true);
      else rcPanel('o agente terminou', 'sua vez de trabalhar');
    }
    else if(rc.dead){
      if(rc.cp) rcPanel(rc.deadMsg || 'você caiu', 'voltando pra bandeira · tokens mantidos');
      else rcPanel(rc.deadMsg || 'você caiu', 'recorde ' + rcBest);
    }
    ctx.textAlign='left'; ctx.textBaseline='alphabetic';
  }
  ...
  if(cv && cv.addEventListener){
    cv.addEventListener('pointerdown', function(e){ if(mode==='recreio'){ rcJump(); if(e && e.preventDefault) e.preventDefault(); } });
    cv.addEventListener('mousedown',  function(e){ if(mode==='recreio'){ rcJump(); if(e && e.preventDefault) e.preventDefault(); } });
  }
  ...
  function loop(now){
    ...
    if(acc >= 120){ acc = 0;
      ...
      if(seasonId!=null){ recomputePop(); evalMarcos(); evalWishes(); buildSpecials(now); maybeSendCity(now); }
      updateHud(now);
