...
//   reporter.report({ seasonId, tokens, cost, residents, buildings });
//
...
// Sanitiza o retrato da cidade vindo do renderer para o formato do contrato do placar:
//   city: { v:1, seed:<uint32>, buildings:<int>, pop:<int>,
//           types:{<tipoEspecial>:<contagem>,...}, marcos:[...], era:<int> }
// Descarta tudo que não bate no shape e garante o JSON <= 2KB (corta types/marcos se
// preciso). Devolve null se não houver cidade válida — o POST simplesmente omite `city`.
const CITY_MAX_BYTES = 2048;
function sanitizeCity(c) {
  if (!c || typeof c !== "object") return null;
  const types = {};
  if (c.types && typeof c.types === "object") {
    for (const k of Object.keys(c.types)) {
      if (/^[a-z0-9-]{1,20}$/.test(k)) types[k] = nonNeg(c.types[k]);
    }
  }
  let marcos = Array.isArray(c.marcos)
    ? c.marcos.filter((s) => typeof s === "string" && /^[a-z0-9-]{1,20}$/.test(s)).slice(0, 24)
    : [];
  let out = {
    v: 1,
    seed: nonNeg(c.seed),
    buildings: nonNeg(c.buildings),
    pop: nonNeg(c.pop),
    types: types,
    marcos: marcos,
    era: nonNeg(c.era),
  };
  // Guarda-dura de tamanho: se estourar 2KB, poda marcos e depois types até caber.
  while (Buffer.byteLength(JSON.stringify(out)) > CITY_MAX_BYTES) {
    if (out.marcos.length) out.marcos = out.marcos.slice(0, -1);
    else {
      const keys = Object.keys(out.types);
      if (!keys.length) break;
      delete out.types[keys[keys.length - 1]];
    }
  }
  return out;
}
...
function createReporter({ configPath }) {
  ...
  function report(snapshot) {
    try {
      ...
      const city = sanitizeCity(snapshot && snapshot.city);
      if (city) body.city = city; // só anexa quando o renderer já mandou a cidade
      const profile = shapeProfile(cfg); // personalização opcional (cityName/motto/accent)
      if (profile) body.profile = profile;
      // BREAKDOWN DIÁRIO: 7 dias UTC reais -> heatmap "CITY LIGHTS" da semana. Sem ele, o
      // servidor preserva os snapshots que já tem (report antigo não apaga).
      const daily = shapeDailyTokens(snapshot && snapshot.dailyTokens);
      if (daily) body.dailyTokens = daily;
      // SETUP (opt-in): só entra no POST se shareSetup e o blob for válido/pequeno.
      if (cfg.shareSetup) {
...
module.exports = { createReporter, sanitizeCity, shapeSetup, shapeDailyTokens };
